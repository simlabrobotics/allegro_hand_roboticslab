...
#include "rDeviceERHandKvaserCAN.h"
#include "canAPI.h"
#include "canDef.h"

static unsigned int __stdcall ioThread(void* inst)
{
	rDeviceERHandKvaserCAN* devCAN = (rDeviceERHandKvaserCAN*)inst;
...
RD_IMPLE_FACTORY(ERHandKvaserCAN)

rDeviceERHandKvaserCAN::rDeviceERHandKvaserCAN() 
: _jdof(-1)
, _errCount(0)
, _ioThread(0)
, _ioThreadRun(false)
...
void rDeviceERHandKvaserCAN::onCreate(const rDeviceContext& rdc)
{
...
void rDeviceERHandKvaserCAN::onInit()
{
...
void rDeviceERHandKvaserCAN::onTerminate()
{
	enable(false);
	RD_DEVICE_CLASS(Base)::onTerminate();
}

int rDeviceERHandKvaserCAN::readDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
int rDeviceERHandKvaserCAN::writeDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
void rDeviceERHandKvaserCAN::updateWriteValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandKvaserCAN::updateReadValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandKvaserCAN::enable(bool b)
{
	int ret;
...
int rDeviceERHandKvaserCAN::command(int cmd, int arg, void* udata, int port)
{
	switch (cmd)
	{
	case CAN_CMD_ENABLE:
...
void rDeviceERHandKvaserCAN::initParams()
{
	const TCHAR* prop;
