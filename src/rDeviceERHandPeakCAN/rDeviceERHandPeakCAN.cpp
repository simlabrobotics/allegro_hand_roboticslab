...
#include "rDeviceERHandPeakCAN.h"
#include "canAPI.h"
#include "canDef.h"
...
RD_IMPLE_FACTORY(ERHandPeakCAN)

rDeviceERHandPeakCAN::rDeviceERHandPeakCAN() 
: _jdof(-1)
, _errCount(0)
, _ioThread(0)
, _ioThreadRun(false)
...
void rDeviceERHandPeakCAN::onCreate(const rDeviceContext& rdc)
{
...
void rDeviceERHandPeakCAN::onInit()
{
...
void rDeviceERHandPeakCAN::onTerminate()
{
	enable(false);
	RD_DEVICE_CLASS(Base)::onTerminate();
}

int rDeviceERHandPeakCAN::readDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
int rDeviceERHandPeakCAN::writeDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
void rDeviceERHandPeakCAN::updateWriteValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandPeakCAN::updateReadValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandPeakCAN::enable(bool b)
{
	int ret;
...
int rDeviceERHandPeakCAN::command(int cmd, int arg, void* udata, int port)
{
	switch (cmd)
	{
	case CAN_CMD_ENABLE:
...
void rDeviceERHandPeakCAN::initParams()
{
	const TCHAR* prop;
