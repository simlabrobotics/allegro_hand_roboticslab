...
#include "rDeviceERHandSoftingCAN.h"
#include "canAPI.h"
#include "canDef.h"

static unsigned int __stdcall ioThread(void* inst)
{
	rDeviceERHandSoftingCAN* devCAN = (rDeviceERHandSoftingCAN*)inst;
...
RD_IMPLE_FACTORY(ERHandSoftingCAN)

rDeviceERHandSoftingCAN::rDeviceERHandSoftingCAN() 
: _jdof(-1)
, _errCount(0)
, _ioThread(0)
, _ioThreadRun(false)
...
void rDeviceERHandSoftingCAN::onCreate(const rDeviceContext& rdc)
{
...
void rDeviceERHandSoftingCAN::onInit()
{
...
void rDeviceERHandSoftingCAN::onTerminate()
{
	enable(false);
	RD_DEVICE_CLASS(Base)::onTerminate();
}

int rDeviceERHandSoftingCAN::readDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
int rDeviceERHandSoftingCAN::writeDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
void rDeviceERHandSoftingCAN::updateWriteValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandSoftingCAN::updateReadValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandSoftingCAN::enable(bool b)
{
	int ret;
...
int rDeviceERHandSoftingCAN::command(int cmd, int arg, void* udata, int port)
{
	switch (cmd)
	{
	case CAN_CMD_ENABLE:
...
void rDeviceERHandSoftingCAN::initParams()
{
	const TCHAR* prop;
