...
#include "rDeviceERHandESDCAN.h"
#include "canAPI.h"
#include "canDef.h"

static unsigned int __stdcall ioThread(void* inst)
{
	rDeviceERHandESDCAN* devCAN = (rDeviceERHandESDCAN*)inst;
...
RD_IMPLE_FACTORY(ERHandESDCAN)

rDeviceERHandESDCAN::rDeviceERHandESDCAN() 
: _jdof(-1)
, _errCount(0)
, _ioThread(0)
, _ioThreadRun(false)
...
void rDeviceERHandESDCAN::onCreate(const rDeviceContext& rdc)
{
...
void rDeviceERHandESDCAN::onInit()
{
...
void rDeviceERHandESDCAN::onTerminate()
{
	enable(false);
	RD_DEVICE_CLASS(Base)::onTerminate();
}

int rDeviceERHandESDCAN::readDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
int rDeviceERHandESDCAN::writeDeviceValue(void* buffer, int len, int port)
{
	if (!_vars)
		return 0;
...
void rDeviceERHandESDCAN::updateWriteValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandESDCAN::updateReadValue(rTime time)
{
	if (!_vars)
		return;
...
void rDeviceERHandESDCAN::enable(bool b)
{
	int ret;
...
int rDeviceERHandESDCAN::command(int cmd, int arg, void* udata, int port)
{
	switch (cmd)
	{
	case CAN_CMD_ENABLE:
...
void rDeviceERHandESDCAN::initParams()
{
	const TCHAR* prop;
